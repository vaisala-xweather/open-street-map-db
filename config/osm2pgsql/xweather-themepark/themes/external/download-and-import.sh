...
download() {
    local layer="$1"
    wget --continue --quiet "https://osmdata.openstreetmap.de/download/$layer.zip"
}
...
import_dataset() {
    ...
    case "$dataset" in
        oceans)
            ...
            ;;
        continents)
            import "$db" "${table}_low" simplified-land-polygons-complete-3857 simplified_land_polygons
            import "$db" "${table}" land-polygons-split-3857 land_polygons
            ;;
        coastlines)
