...
BEGIN
  ...
  LOOP
    WITH
    merged AS
        (SELECT kind, link, tunnel, bridge, surface,
                service, layer, ref, ref_rows, ref_cols, z_order, minzoom,
                ST_LineMerge(ST_Collect(geom)) AS geom
        FROM {schema}.{prefix}streets_med_interim
            WHERE geom && cell
            GROUP BY kind, link, tunnel, bridge, surface,
                    service, layer, ref, ref_rows, ref_cols, z_order, minzoom),
    simplified AS
        (SELECT 1, kind, link, tunnel, bridge,
            surface, service, layer, ref,
            ref_rows, ref_cols, z_order, minzoom,
            ST_Simplify((ST_Dump(geom)).geom, 20) AS geom
        FROM merged)
    INSERT INTO {schema}.{prefix}streets_med_new (way_id, kind, link, tunnel, bridge,
                            surface, service, layer, ref,
...
WITH
merged AS
    (SELECT kind, ref, minzoom, ST_LineMerge(ST_Collect(geom)) AS geom
        FROM {schema}.{prefix}streets_low_interim GROUP BY kind, ref, minzoom),
simplified AS
    (SELECT 1, kind, ref, minzoom,
            ST_Simplify((ST_Dump(geom)).geom, 20) AS geom FROM merged)
INSERT INTO {schema}.{prefix}streets_low_new (way_id, kind, ref, minzoom, geom)
    SELECT * FROM simplified WHERE geom IS NOT NULL]]),
